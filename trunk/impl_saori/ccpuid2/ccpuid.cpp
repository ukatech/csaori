...
void CSAORI::exec(const CSAORIInput& in,CSAORIOutput& out)
{
	...
	if (in.args[0] == L"platform") {				// platform: basename of caller exe name
		...
		// New CPU functions
			...
			char tmptxt[30];
			string outtxt;
			sprintf_s(tmptxt,sizeof(tmptxt),"%d %d %d %d",uL1DCSize.iL1DCSize,uL1ICSize.iL1ICSize,uL2Size.iL2Size,uL3Size.iL3Size);
			outtxt = tmptxt;
			out.result_code = SAORIRESULT_OK;
		...
		// CPU Speed functions (saori_cpuid compatible)
			...
			char tmptxt[10];
			string outtxt;
			sprintf_s(tmptxt,sizeof(tmptxt),"%d",(int)GetCPUSpeed(100));
			outtxt = tmptxt;
			out.result_code = SAORIRESULT_OK;
			out.result = SAORI_FUNC::MultiByteToUnicode(outtxt);
		}
			...
			char tmptxt[10];
			string outtxt;
			sprintf_s(tmptxt,sizeof(tmptxt),"%.3f",GetCPUSpeed(250));
			outtxt = tmptxt;
			out.result_code = SAORIRESULT_OK;
			out.result = SAORI_FUNC::MultiByteToUnicode(outtxt);
		}
